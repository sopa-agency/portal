...
export type ConnectClient = { id: string; label: string; note?: Both; blocks: (origin: string, token: string) => ConnectBlock[] };
...
export const CLIENTS: ConnectClient[] = [
  {
    id: "claude-code",
    label: "Claude Code",
    blocks: (o, t) => [{ text: `claude mcp add --transport http sopa ${mcpUrl(o)} \\\n  --header "Authorization: Bearer ${t}"` }],
  },
  {
    id: "codex",
    label: "Codex",
    ...
    blocks: (o, t) => [
      { text: `export ${TOKEN_ENV}="${t}"\n\ncodex mcp add sopa --url ${mcpUrl(o)} \\\n  --bearer-token-env-var ${TOKEN_ENV}` },
      { title: { pt: "Ou direto em ~/.codex/config.toml", en: "Or straight into ~/.codex/config.toml" }, text: `[mcp_servers.sopa]\nurl = "${mcpUrl(o)}"\nbearer_token_env_var = "${TOKEN_ENV}"` },
    ],
  },
  {
    id: "openclaw",
    label: "OpenClaw",
    note: { pt: "O add testa a conexão antes de gravar. O reload faz os agentes pegarem o servidor no próximo turno.", en: "add probes the connection before saving. reload makes the agents pick the server up on their next turn." },
    blocks: (o, t) => [{ text: `openclaw mcp add sopa --url ${mcpUrl(o)} \\\n  --transport streamable-http \\\n  --header "Authorization=Bearer ${t}"\n\nopenclaw mcp reload` }],
  },
  {
    id: "gemini",
    label: "Gemini CLI",
    blocks: (o, t) => [{ text: `gemini mcp add --transport http --scope user sopa ${mcpUrl(o)} \\\n  -H "Authorization: Bearer ${t}"` }],
  },
  {
    id: "cursor",
    label: "Cursor",
    ...
    blocks: (o, t) => [{ text: json({ mcpServers: { sopa: { url: mcpUrl(o), headers: { Authorization: `Bearer ${t}` } } } }) }],
  },
  {
    id: "vscode",
    label: "VS Code",
    ...
    blocks: (o, t) => [{ text: json({ servers: { sopa: { type: "http", url: mcpUrl(o), headers: { Authorization: `Bearer ${t}` } } } }) }],
  },
  {
    id: "claude-desktop",
    ...
    blocks: (o, t) => [{ text: json({ mcpServers: { sopa: { command: "npx", args: ["-y", "mcp-remote", mcpUrl(o), "--header", "Authorization:${AUTH_HEADER}"], env: { AUTH_HEADER: `Bearer ${t}` } } } }) }],
  },
];
