...
const en = {
  ...
  treasury: {
    ...
    views: {
      ...
      liquidity: "Liquidity",
      liquidityHint: "what you can move today",
      whoHolds: "Who holds what",
      walletsIn: (name: string) => `Wallets in ${name}`,
      assetMix: "Asset mix",
      topShown: (shown: number, total: number) => `${total} assets, top ${shown} shown`,
      expandAll: "Expand all",
      collapseAll: "Collapse all",
      walletsNote: (n: number, p: number) =>
        `${n} wallet${n === 1 ? "" : "s"} across ${p} project${p === 1 ? "" : "s"}. Open one to see every asset it holds.`,
      dustHidden: (n: number, sum: string) =>
        `${n} more holding${n === 1 ? " is" : "s are"} under $5 or unpriced${sum ? ` — the priced ones add up to ${sum}` : ""}.`,
      showDust: "Show dust",
      hideDust: "Hide dust",
      kpi: {
        treasury: "Treasury",
        health: "Health",
        runway: "Runway",
        custody: "Custody split",
        months: "months",
        counting: (usd: string) => `counting ${usd}/mo of costs booked to this scope`,
        noneFiled: "no recurring costs are booked to this scope — they run through SOPA",
        healthy: "Healthy",
        healthyNote: "The cash covers well over a year at the current pace.",
        watch: "Watch it",
        watchNote: "Under a year of cash — worth following closely.",
        tight: "Tight",
        tightNote: "Under six months at the current pace.",
        noCosts: "No costs",
        incomplete: "Incomplete",
        incompleteNote: "Part of the treasury didn't read — no total, no verdict.",
      },
    },
    revenue: {
    ...
    capital: {
      ...
      bridge: {
        title: "MOR on Arbitrum",
        hint: "Claims mint MOR on Arbitrum; the restake and the swapper live on Base. This moves the Safe's MOR across.",
        waiting: (mor: string) => `${mor} MOR waiting on Arbitrum`,
        none: "No MOR waiting on Arbitrum.",
        quote: "Quote the bridge",
        quoting: "Quoting both routes…",
        viaSwapsPro: "via swaps.pro",
        viaOft: "via LayerZero (the token bridges itself)",
        receives: (n: string) => `receives ${n} MOR`,
        floor: (n: string) => `floor ${n}`,
        exact: "exact amount, no slippage",
        cost: (s: string) => `cost ${s}`,
        cheaper: "cheaper for this amount",
        propose: "Propose in the Safe",
        proposing: "Proposing…",
        queued: "Simulated as the Safe just now: it executes. Queued on Arbitrum; it needs an owner's signature.",
        queueLink: "Open the Safe queue",
        eth: (arb: string, main: string, claims: string) => `ETH in the Safe: ${arb} on Arbitrum · ${main} on mainnet (≈ ${claims} claims)`,
        ethLow: "Top up ETH on mainnet before the next claim.",
        firm: "The swaps.pro route is quoted with 3% slippage and carries an on-chain deadline of about 30 minutes, but MOR moves fast: propose and sign within minutes, in the same sitting. The proposal is simulated before it is queued; if the market moved, you get told here instead of a GS013 in the Safe. The LayerZero route has no deadline and no slippage.",
        validUntil: (hhmm: string, min: number) => `valid until ${hhmm} (${min} min)`,
        expired: "this quote's deadline has passed — quote again",
        signNow: (hhmm: string) => `Sign and execute before ${hhmm}, or it reverts.`,
        replaced: "It replaces the previous bridge proposal at the same nonce; execute this one and the old one dies.",
        ...
        execNow: "Execute now with the wallet",
        execHint: "This Safe needs one signature on Arbitrum: an owner wallet connected here quotes, signs and executes in one go — the only way the swaps.pro route fits its window.",
        execConnecting: "Connecting the wallet…",
        execBuilding: "Quoting and simulating…",
        execSigning: "Waiting for the wallet…",
        execMining: "Sent — waiting for Arbitrum…",
        execDone: "Executed on Arbitrum.",
        execFailed: "The transaction was mined but reverted.",
        execNotOwner: (who: string) => `${who} is not an owner of this Safe on Arbitrum. Connect an owner wallet.`,
        execNoWallet: "No wallet in this browser.",
        arrival: "The MOR lands on Base in a few minutes.",
        checkArrival: "Check the Base balance",
        onBase: (mor: string) => `MOR on Base now: ${mor}`,
        track: "Track the bridge",
        onArbiscan: "Arbiscan",
      },
      steps: {
        claim: "Claim",
        claimHint: "Mints the MOR accrued by the deposit — on Arbitrum, into this same Safe, never to a split. Payable: the Safe pays the LayerZero fee from its mainnet ETH. Locked for 7 days after every deposit.",
        bridge: "Bridge",
        bridgeHint: "The MOR lands on Arbitrum; the restake and the swapper live on Base. LayerZero is the token bridging itself: no aggregator, no deadline, exact amount. swaps.pro is a swap-and-bridge route quoted with 3% slippage that must be executed within minutes.",
        restake: "Restake",
        restakeHint: "Deposits the Safe's MOR on Base into the Builders subnet, in one Safe batch (withdraw from the Warehouse + approve + deposit). Every deposit re-locks the whole position for 7 days.",
        nothingToClaim: "Nothing to claim yet.",
        claimAction: "Propose claim",
        queuedMainnet: "Queued on mainnet.",
        unlocksAt: (d: string) => `unlocks ${d}`,
        ethFor: (eth: string, n: string) => `${eth} ETH on mainnet · ≈ ${n} claims`,
        onArbitrum: (m: string) => `${m} MOR on Arbitrum`,
        nothingOnArbitrum: "Nothing waiting on Arbitrum.",
        bring: "Bring to Base",
        viaLz: "LayerZero",
        viaSp: "swaps.pro",
        useOther: (via: string, receives: string, cost: string) => `or ${via}: ${receives}${cost ? ` · ${cost}` : ""}`,
        spUnavailable: "swaps.pro didn't return a route right now.",
        run: "Execute with the wallet",
        willPropose: "Goes to the Safe queue; an owner signs there.",
        wrongChain: "The wallet is not on Arbitrum.",
        cancelled: "Cancelled in the wallet.",
        onBase: (m: string) => `${m} MOR on Base`,
        restakeAction: "Propose restake",
        stakeMor: "Stake in the Gnars subnet",
        proposingStake: "Proposing…",
        restakeQueued: (m: string) => `Queued: stake ${m} MOR in the Gnars subnet. Two signatures on Base.`,
        restakePending: "Destination to be decided.",
      },
    },
...
const pt: typeof en = {
  ...
  treasury: {
    ...
    views: {
      ...
      liquidity: "Liquidez",
      liquidityHint: "o que dá para mover hoje",
      whoHolds: "Quem tem o quê",
      walletsIn: (name: string) => `Carteiras em ${name}`,
      assetMix: "Mix de ativos",
      topShown: (shown: number, total: number) => `${total} ativos, os ${shown} maiores`,
      expandAll: "Expandir tudo",
      collapseAll: "Recolher tudo",
      walletsNote: (n: number, p: number) =>
        `${n} carteira${n === 1 ? "" : "s"} em ${p} projeto${p === 1 ? "" : "s"}. Abra uma para ver cada ativo.`,
      dustHidden: (n: number, sum: string) =>
        `+${n} posiç${n === 1 ? "ão" : "ões"} abaixo de US$ 5 ou sem preço${sum ? ` — as precificadas somam ${sum}` : ""}.`,
      showDust: "Ver poeira",
      hideDust: "Esconder poeira",
      kpi: {
        treasury: "Tesouro",
        health: "Saúde",
        runway: "Runway",
        custody: "Onde está",
        months: "meses",
        counting: (usd: string) => `contando ${usd}/mês de custo lançado neste escopo`,
        noneFiled: "nenhum custo recorrente lançado neste escopo — os gastos correm pela SOPA",
        healthy: "Saudável",
        healthyNote: "O caixa cobre bem mais de um ano no ritmo atual.",
        watch: "De olho",
        watchNote: "Menos de um ano de caixa — vale acompanhar de perto.",
        tight: "Apertado",
        tightNote: "Menos de seis meses no ritmo atual.",
        noCosts: "Sem custo",
        incomplete: "Incompleto",
        incompleteNote: "Parte do tesouro não leu — sem total não há veredito.",
      },
    },
    revenue: {
    ...
    capital: {
      ...
      bridge: {
        title: "MOR na Arbitrum",
        hint: "O claim minta o MOR na Arbitrum; o restake e o swapper vivem na Base. Isto traz o MOR do Safe para cá.",
        waiting: (mor: string) => `${mor} MOR esperando na Arbitrum`,
        none: "Nenhum MOR esperando na Arbitrum.",
        quote: "Cotar a ponte",
        quoting: "Cotando as duas rotas…",
        viaSwapsPro: "pelo swaps.pro",
        viaOft: "pelo LayerZero (a própria token é a ponte)",
        receives: (n: string) => `recebe ${n} MOR`,
        floor: (n: string) => `piso ${n}`,
        exact: "quantia exata, sem slippage",
        cost: (s: string) => `custa ${s}`,
        cheaper: "mais barata para este valor",
        propose: "Propor no Safe",
        proposing: "Propondo…",
        queued: "Simulada como o Safe agora mesmo: executa. Na fila da Arbitrum; falta a assinatura de um dono.",
        queueLink: "Abrir a fila do Safe",
        eth: (arb: string, main: string, claims: string) => `ETH no Safe: ${arb} na Arbitrum · ${main} na mainnet (≈ ${claims} claims)`,
        ethLow: "Manda ETH pro Safe na mainnet antes do próximo claim.",
        firm: "A rota do swaps.pro é cotada com 3% de slippage e carrega um prazo on-chain de uns 30 minutos, mas o MOR anda rápido: proponha e assine em minutos, na mesma sentada. A proposta é simulada antes de entrar na fila; se o mercado andou, o aviso aparece aqui em vez de um GS013 no Safe. A rota LayerZero não tem prazo nem slippage.",
        validUntil: (hhmm: string, min: number) => `vale até ${hhmm} (${min} min)`,
        expired: "o prazo desta cotação passou — cota de novo",
        signNow: (hhmm: string) => `Assina e executa antes das ${hhmm}, senão reverte.`,
        replaced: "Ela substitui a proposta de ponte anterior no mesmo nonce; executa esta e a velha morre.",
        ...
        execNow: "Executar agora com a carteira",
        execHint: "Este Safe pede uma assinatura na Arbitrum: a carteira de um dono conectada aqui cota, assina e executa de uma vez — o único jeito de a rota do swaps.pro caber na janela dela.",
        execConnecting: "Conectando a carteira…",
        execBuilding: "Cotando e simulando…",
        execSigning: "Esperando a carteira…",
        execMining: "Enviada — esperando a Arbitrum…",
        execDone: "Executada na Arbitrum.",
        execFailed: "A transação minerou mas reverteu.",
        execNotOwner: (who: string) => `${who} não é dono deste Safe na Arbitrum. Conecta a carteira de um dono.`,
        execNoWallet: "Nenhuma carteira neste navegador.",
        arrival: "O MOR cai na Base em alguns minutos.",
        checkArrival: "Conferir o saldo na Base",
        onBase: (mor: string) => `MOR na Base agora: ${mor}`,
        track: "Acompanhar a ponte",
        onArbiscan: "Arbiscan",
      },
      steps: {
        claim: "Claim",
        claimHint: "Minta o MOR rendido pelo depósito — na Arbitrum, neste mesmo Safe, nunca num split. É payable: o Safe paga a taxa LayerZero com o ETH dele na mainnet. Trava 7 dias depois de cada depósito.",
        bridge: "Ponte",
        bridgeHint: "O MOR cai na Arbitrum; o restake e o swapper vivem na Base. LayerZero é a própria token atravessando: sem agregador, sem prazo, quantia exata. swaps.pro é uma rota de swap e ponte, cotada com 3% de slippage, que precisa executar em minutos.",
        restake: "Restake",
        restakeHint: "Deposita o MOR do Safe na Base no subnet dos Builders, num batch só (saque do Warehouse + approve + deposit). Cada depósito retrava a posição inteira por 7 dias.",
        nothingToClaim: "Nada a reclamar ainda.",
        claimAction: "Propor claim",
        queuedMainnet: "Na fila da mainnet.",
        unlocksAt: (d: string) => `libera ${d}`,
        ethFor: (eth: string, n: string) => `${eth} ETH na mainnet · ≈ ${n} claims`,
        onArbitrum: (m: string) => `${m} MOR na Arbitrum`,
        nothingOnArbitrum: "Nada esperando na Arbitrum.",
        bring: "Trazer para a Base",
        viaLz: "LayerZero",
        viaSp: "swaps.pro",
        useOther: (via: string, receives: string, cost: string) => `ou ${via}: ${receives}${cost ? ` · ${cost}` : ""}`,
        spUnavailable: "O swaps.pro não devolveu rota agora.",
        run: "Executar com a carteira",
        willPropose: "Vai para a fila do Safe; um dono assina lá.",
        wrongChain: "A carteira não está na Arbitrum.",
        cancelled: "Cancelado na carteira.",
        onBase: (m: string) => `${m} MOR na Base`,
        restakeAction: "Propor restake",
        stakeMor: "Stake no subnet da Gnars",
        proposingStake: "Propondo…",
        restakeQueued: (m: string) => `Na fila: stake de ${m} MOR no subnet da Gnars. Duas assinaturas na Base.`,
        restakePending: "Destino a decidir.",
      },
    },
    members: {
