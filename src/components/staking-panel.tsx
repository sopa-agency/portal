    ...
    <section className="rounded-2xl border border-success/40 bg-surface p-5">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <h2 className="flex items-center gap-2 text-lg font-semibold tracking-tight text-foreground">
          <PiggyBank className="h-4 w-4 text-success" /> {t.title}
        </h2>
        <span className="font-mono text-[11px] text-foreground-faint">Moonwell Flagship USDC</span>
      </div>

      {position.failed ? (
        <ReadFailed>{dict.ui.state.readFailed}</ReadFailed>
      ) : (
        <div className="grid grid-cols-3 gap-4">
          <div>
            <div className="text-[10px] uppercase tracking-wider text-foreground-faint">{t.saved}</div>
            <div className="text-sm font-semibold tabular-nums text-foreground">{usd(position.valueUsd)}</div>
          </div>
          <div>
            <div className="text-[10px] uppercase tracking-wider text-foreground-faint">{t.apyYear}</div>
            <div className="text-sm font-semibold tabular-nums text-success">
              {position.apy != null ? pct(position.apy) : "—"}
            </div>
          </div>
          <div>
            <div className="text-[10px] uppercase tracking-wider text-foreground-faint">{t.perMonth}</div>
        ...
        <div className="mt-4 border-t border-border pt-4">
          ...
          <div className="flex flex-wrap gap-2">
            <input
              ...
              inputMode="decimal"
              aria-label={mode === "stake" ? t.amountStake : t.amountUnstake}
              placeholder="USDC"
