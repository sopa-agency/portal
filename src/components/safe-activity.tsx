...
function TxRow({ tx, chainId }: { tx: SafeTxView; chainId: number }) {
  return (
    <li className="flex items-center gap-2 py-1.5 text-xs">
      ...
      {tx.executed ? (
        tx.success === false ? (
          <span className="flex items-center gap-1 text-danger"><XCircle className="h-3 w-3" /> falhou</span>
        ) : (
          <a
          ...
          >
            <CheckCircle2 className="h-3 w-3" /> executado <ExternalLink className="h-2.5 w-2.5" />
          </a>
        )
      ) : (
      ...
      <span className="shrink-0 text-foreground-faint">{fmtDate(tx.submissionDate)}</span>
    </li>
  );
}

/** Read-only Safe transaction activity (pending queue + recent executed) per Safe wallet. */
export function SafeActivity({ safes }: { safes: SafeActivityItem[] }) {
  if (safes.length === 0) return null;
  return (
    <section className="space-y-3">
      <h2 className="text-sm font-semibold uppercase tracking-[0.14em] text-foreground-subtle">Transações do Safe</h2>
            ...
            <div key={s.address} className="rounded-xl border border-border bg-surface p-3">
              <div className="mb-2 flex items-center justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="font-mono text-[11px] text-foreground-faint">{short(s.address)} · threshold {s.activity.threshold}</p>
                </div>
              ...
              {s.activity.queued.length > 0 ? (
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-wide text-warning">Pendentes ({s.activity.queued.length})</p>
                  <ul className="divide-y divide-border/60">
                    {s.activity.queued.map((tx) => <TxRow key={tx.safeTxHash} tx={tx} chainId={s.chainId} />)}
                  </ul>
                </div>
              ) : (
                <p className="text-xs text-foreground-faint">Sem transações pendentes.</p>
              )}
              ...
              {s.activity.history.length > 0 && (
                <details className="mt-2 group">
                  <summary className="cursor-pointer list-none text-[11px] text-foreground-muted hover:text-foreground">
                    Recentes ({s.activity.history.length}) ▾
                  </summary>
                  <ul className="mt-1 divide-y divide-border/60">
                    {s.activity.history.map((tx) => <TxRow key={tx.safeTxHash} tx={tx} chainId={s.chainId} />)}
                  </ul>
                </details>
              )}
            </div>
