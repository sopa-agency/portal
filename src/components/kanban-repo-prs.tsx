  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        aria-label="Mostrar pull requests abertos dos repositórios"
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 max-h-[70vh] w-[min(24rem,calc(100vw-2rem))] overflow-y-auto rounded-xl border border-border bg-surface p-2 shadow-xl">
          {prs.length === 0 ? (
            <p className="px-2 py-6 text-center text-xs text-foreground-muted">
              Nenhum PR aberto nos repositórios do projeto.
            </p>
          ) : (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-0.5 flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-[11px] text-foreground-subtle">
                        ...
                        <span className="text-foreground-faint">· {relTime(pr.updatedAt)}</span>
                        {pr.draft && <span className="text-foreground-faint">· rascunho</span>}
                      </span>
                    </span>
                  </a>
                </li>
